...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeSocketConnection() {
        // Connect to the server using current domain
    ...
    function joinChannel(channelId) {
        ...
        socket.emit('joinChannel', { channelId }, (response) => {
            if (response.success) {
                ...
                showNotification(response.error || 'Failed to join channel', 'error');
            }
        });
    }
    ...
    function leaveCurrentChannel(isDisconnect = false) {
        ...
        if (!isDisconnect && socket && socket.connected) {
    ...
    function displayEmojiAnimation(emoji, username) {
        ...
        for (let i = 0; i < participants.length; i++) {
            if (participants[i].textContent.includes(username)) {
                const rect = participants[i].getBoundingClientRect();
    ...
    // Add showNotification to window for other modules to use
